...
function renameSudir(from, to) {
      ...
      fs.readdirSync(dir).forEach(function (file) {
        ...
        try {
          if (path.basename(file) === from) {
            var fileTo = path.combine(path.dirname(file), to);
            fs.renameSync(file, fileTo);
            newlyConverted.push(fileTo);
          }
          else if (path.basename(file) === to) {
            alreadyConverted.push(file);
          }
        }
        catch (error) {
  ...
  if (newlyConverted.length)
    console.log('Renamed:' + newlyConverted.map(function (toDir) { return '  ' + toDir; }));
  if (alreadyConverted.length)
    console.log('Ready:' + alreadyConverted.map(function (toDir) { return '  ' + toDir; }));
